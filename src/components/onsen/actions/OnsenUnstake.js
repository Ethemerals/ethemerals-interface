...
const OnsenUnstake = ({ contractOnsen, tokenId }) => {
	...
	const handleUnstake = async () => {
		if (contractOnsen) {
			...
			try {
				...
				sendTx(tx.hash, 'leave stake', true, ['account', 'account_onsen']);
			} catch (error) {
				setIsErrorOpen(true);
	...
	return (
		<div>
			<button onClick={handleUnstake} data-tip data-for="ttUnstake" className={`mr-1 text-green-500 opacity-100 hover:text-green-700 cursor-pointer transition duration-300`}>
				<SVGUnstake />
			</button>

			<ReactTooltip id="ttUnstake" type="success" effect="solid">
				<span>Leave Onsen</span>
			</ReactTooltip>

			{isConfirmationOpen && <WaitingConfirmation toggle={toggleConfirmation} message={`Leave ${tokenId} from Onsen!`} />}
			{isErrorOpen && <ErrorDialogue toggle={toggleError} message={errorMsg} />}
		</div>
	);
};

export default OnsenUnstake;
