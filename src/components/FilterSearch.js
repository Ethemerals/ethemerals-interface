...
const FilterSearch = ({ data, setFilterList, filterList, keys, filterByText = '' }) => {
	...
	return (
		<div className="text-black m-2">
			<div className="flex p-2 bg-white rounded-lg shadow">
				<span className="text-gray-400">
					<svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
						<path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
					</svg>
				</span>
				<input className="w-48 px-2 outline-none" onChange={(e) => search(e.target.value)} value={term} placeholder={`Filter ${filterByText}`} />
			</div>
